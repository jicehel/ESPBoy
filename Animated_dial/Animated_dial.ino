...
#define DIAL_CENTRE_X 64
#define DIAL_CENTRE_Y 64

#include "ESPboyInit.h"
ESPboyInit myESPboy;       // Invoke custom library

TFT_eSprite needle = TFT_eSprite(&myESPboy.tft); // Sprite object for needle
TFT_eSprite spr    = TFT_eSprite(&myESPboy.tft); // Sprite for meter reading
...
bool tft_output(int16_t x, int16_t y, uint16_t w, uint16_t h, uint16_t* bitmap)
{
  ...
  if ( y >= myESPboy.tft.height() ) return 0;

  // This function will clip the image block rendering automatically at the TFT boundaries
  myESPboy.tft.pushImage(x, y, w, h, bitmap);
...
void setup()   {
  ...
  myESPboy.begin("Animated_dial");
  myESPboy.tft.fillScreen(TFT_BLACK);

  // Draw the dial
  TJpgDec.drawJpg(0, 0, dial, sizeof(dial));
  myESPboy.tft.drawCircle(DIAL_CENTRE_X, DIAL_CENTRE_Y, NEEDLE_RADIUS-NEEDLE_LENGTH, TFT_DARKGREY);
  ...
  // Plot the label text
  myESPboy.tft.setTextColor(TFT_WHITE, bg_color);
  myESPboy.tft.setTextDatum(MC_DATUM);
  myESPboy.tft.drawString("(degrees)", DIAL_CENTRE_X, DIAL_CENTRE_Y + 48, 2);

  // Define where the needle pivot point is on the TFT before
  // creating the needle so boundary calculation is correct
  myESPboy.tft.setPivot(DIAL_CENTRE_X, DIAL_CENTRE_Y);
...
void createNeedle(void)
{
  ...
  needle.fillRect(0, 0, NEEDLE_WIDTH, NEEDLE_LENGTH, NEEDLE_COLOR1);
  needle.fillRect(1, 1, NEEDLE_WIDTH-2, NEEDLE_LENGTH-2, NEEDLE_COLOR2);
  ...
  while (angle != old_angle || !buffer_loaded) {
    ...
    if ( (old_angle & 1) == 0)
    {
      if (buffer_loaded) {
        ...
        myESPboy.tft.pushRect(min_x, min_y, 1 + max_x - min_x, 1 + max_y - min_y, tft_buffer);
      }

      if ( needle.getRotatedBounds(old_angle, &min_x, &min_y, &max_x, &max_y) )
      {
        // Grab a copy of the area before needle is drawn
        myESPboy.tft.readRect(min_x, min_y, 1 + max_x - min_x, 1 + max_y - min_y, tft_buffer);
        buffer_loaded = true;
      }
